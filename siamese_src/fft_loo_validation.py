...
for train_index, test_index in loo.split(X):
    ...
    hyperparameters_svc = [{'name': 'cost', 'type': 'continuous',
                        ...
                        'domain': (0.00001, 0.1)}]#0-linear, 1-rbf
    opt_hyperparameters_svc = opt_clf.optimize_classifier(X_train, y_train, SVC, hyperparameters_svc)
    if(opt_hyperparameters_svc[1]):
        svm = SVC(C=opt_hyperparameters_svc[0], kernel='rbf', gamma=opt_hyperparameters_svc[2])
    else:
        svm = SVC(C=opt_hyperparameters_svc[0], kernel='linear', gamma=opt_hyperparameters_svc[2])
