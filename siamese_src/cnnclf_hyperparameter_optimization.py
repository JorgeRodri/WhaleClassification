...
def data(population, ids, norm_factor):
    X = np.array(get_population_stft_by_bins(population, ids,
                                             binarize=binarize, n_channels=number_channels, nperseg=nperseg))
            ...
            with session:
                ...
                for prediction in range(len(y_pred)):
                    if(y_pred[prediction] >= 0.5):
                        if(1 == y_test[prediction]):
                            accuracy_mean += 1
                    else:
                        if(0 == y_test[prediction]):
                            accuracy_mean += 1
